...
What's left is **1,525 lint errors**, none of which block the build or tests today —
`npm run lint` simply isn't green yet.

This is not 1,525 independent problems. A `-f json` dump (`npx eslint . -f json`,
92 files affected) shows the errors cluster hard around a small number of root
causes:
...
| 279 | `restrict-template-expressions` | 95.3% in source (numbers/enums in generated script text) |
...
The first two rows (979 errors, **64% of the total**) trace back almost entirely to
one test-writing pattern: `const service = xService as any;`, used to reach private
methods under test. Fixing that pattern in ~15 files is worth doing before touching
anything else — it's the highest-leverage item on this list by a wide margin.
...
### ☐ 1. `restrict-template-expressions` — decide `allowNumber`/`allowRegExp` policy

279 hits, 250 of which are a number or enum member interpolated into a template
literal (e.g. `` `duration=${effect.duration}` ``, `` `${EffectTypeEnum.Hold}` ``).
This is the single most common operation in a WeiDU/BAF script generator — turning
typed values into generated script text — and the strict-preset default
(`allowNumber: false`) treats every one of those 250 as an error.
...
in `eslint.config.mjs`, rather than wrapping ~250 call sites in `String(...)`. This
turns Tier 0 into a one-line config change instead of a 250-site sweep.

**Do not blanket-allow the other 29** — those are `X | undefined` types (see Tier 2
item 5). Interpolating an `undefined` value renders the literal string `"undefined"`
into generated WeiDU script/doc output, which is a real bug class, not noise. Keep
those as hard errors and fix them individually.
