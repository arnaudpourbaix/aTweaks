...
import { Creature, CREATURE_DATA_KEYS, CreatureAdditionalData, CreatureAdjustment, CreatureData } from "../model/final/creature";
import { Effect } from "../model/final/effect";
import { ItemAbilityLocationEnum, ItemAbilityTargetEnum, ItemAbilityTypeEnum, SpellTypeEnum } from "../model/final/enums";
...
import { ItemSlotEnum } from "../model/raw/item";
...
export class WeiduCreatureService extends AbstractWeiduService {
    ...
    generateWeiduScript(creature: Creature): void {
        ...
        fs.writeFileSync(`${path.join(State.modFolder, creature.tpaFile)}.tpa`, content);
    }

    private compileScripts(lines: CodeLine[], creature: Creature) {
        this.add(lines, `COMPILE ~%MOD_FOLDER%/${creature.bafFile}.baf~`);
        if (creature.adjustments.some(a => !!a.summon)) this.add(lines, `COMPILE ~%MOD_FOLDER%/${creature.bafFile}su.baf~`);
        this.add(lines, '');
    }
    ...
    private createItems(lines: CodeLine[], creature: Creature) {
        for (const item of creature.items) {
            if (item.copyFrom) {
                ...
                if (immunity && !immunity.itemSlot) throw new Error(`No file configured for immunity ${item.copyFrom}`);
                this.add(lines, `COPY_EXISTING ~${immunity?.itemSlot?.file ?? item.copyFrom}.ITM~  ~override/${item.file}.ITM~`, 0);
            } else {
            ...
            if (item.name) this.add(lines, `SAY NAME1 ~${item.name}~ SAY NAME2 ~${item.name}~`, 1);
            if (item.description) this.add(lines, `SAY UNIDENTIFIED_DESC ~${item.description.join("\n")}~`, 1);
            if (item.category) this.add(lines, `WRITE_SHORT 0x1c ${item.category}`, 1);
            if (item.animation) this.add(lines, `WRITE_ASCII 0x22 ~${item.animation}~ #2`, 1);
            if (item.proficiency) this.add(lines, `WRITE_BYTE 0x31 ${item.proficiency}`, 1);
            if (item.icon) this.add(lines, `WRITE_ASCII 0x3a ~${item.icon}~ #8`, 1);
            if (item.enchantment) this.add(lines, `WRITE_LONG 0x60 ${item.enchantment}`, 1);
            //this.add(lines, `LPF set_enchantment INT_VAR enchantment = ${item.enchantment} END`, 1);
            if (item.type) {
                if (item.location) this.add(lines, `WRITE_SHORT 0x74 ${item.location}`, 1);
                ...
                if (item.bonusToHit) this.add(lines, `WRITE_SHORT 0x86 ${item.bonusToHit}`, 1);
                if (item.diceSize) this.add(lines, `WRITE_BYTE 0x88 ${item.diceSize}`, 1);
                if (item.diceThrown) this.add(lines, `WRITE_BYTE 0x8a ${item.diceThrown}`, 1);
                if (item.damageBonus) this.add(lines, `WRITE_SHORT 0x8c ${item.damageBonus}`, 1);
                if (item.damageType) this.add(lines, `WRITE_SHORT 0x8e ${item.damageType}`, 1);
                if (item.projectile) this.add(lines, `WRITE_SHORT 0x9c (IDS_OF_SYMBOL (~projectl~ ~${item.projectile}~)) + 1`, 1);
                if (item.type === ItemAbilityTypeEnum.Melee) {
                    this.add(lines, `WRITE_SHORT 0x9e ${item.animationSwing?.overhand ?? '34'}`, 1);
                    this.add(lines, `WRITE_SHORT 0xa0 ${item.animationSwing?.backhand ?? '33'}`, 1);
                    this.add(lines, `WRITE_SHORT 0xa2 ${item.animationSwing?.thrust ?? '33'}`, 1);
            ...
            for (const name of item.immunities) {
                this.add(lines, `LPF ${this.utils.getImmunityFunctionName(name)} END`, 1);
            }
            this.add(lines, '', 0);
        }
    }
    ...
    private createSpells(lines: CodeLine[], creature: Creature) {
        for (const spell of creature.spells) {
            if (spell.copyFrom) {
                this.add(lines, `COPY_EXISTING ~${spell.copyFrom}.SPL~  ~override/${spell.file}.SPL~`, 0);
            } else {
            ...
            if (spell.spellType) this.add(lines, `WRITE_SHORT 0x1c ${spell.spellType}`, 1);
            if (spell.secondaryType) this.add(lines, `WRITE_BYTE 0x27 ${spell.secondaryType}`, 1);
            if (spell.spellLevel) this.add(lines, `WRITE_LONG 0x34 ${spell.spellLevel}`, 1);
            if (spell.location) this.add(lines, `WRITE_SHORT 0x74 ${spell.location}`, 1);
            ...
            if (typeof spell.stringRef === 'number') this.add(lines, `SAY NAME1 @${spell.stringRef} SAY NAME2 @${spell.stringRef}`, 1);
            else if (typeof spell.stringRef === 'string') this.add(lines, `SAY NAME1 ~${spell.stringRef}~ SAY NAME2 ~${spell.stringRef}~`, 1);
            if (spell.description) this.add(lines, `SAY UNIDENTIFIED_DESC ~${spell.description.join("\n")}~`, 1);
            if (spell.projectile) this.add(lines, `WRITE_SHORT 0x98 (IDS_OF_SYMBOL (~projectl~ ~${spell.projectile}~)) + 1`, 1);
            for (const effect of spell.effects) this.addEffect(lines, 1, effect, 'SPL');
            for (const opcode of spell.removeOpcodes)
                this.add(lines, `LPF DELETE_EFFECT INT_VAR match_opcode = ${opcode} END`, 1);
            this.add(lines, '', 0);
        }
    }
    ...
    private createProjectiles(lines: CodeLine[], creature: Creature) {
        for (const projectile of creature.projectiles) {
            this.add(lines, `COPY_EXISTING "${projectile.copyFromFile}.pro" ~override/${projectile.file}.pro~`, 0);
            ...
            if (projectile.type) this.add(lines, `WRITE_SHORT 0x08 ${projectile.type}`, 1);
            if (projectile.speed) this.add(lines, `WRITE_SHORT 0x0a ${projectile.speed}`, 1);
            ...
            if (projectile.fireSound) this.add(lines, `WRITE_ASCII 0x10 ~${projectile.fireSound}~ #8`, 1);
            if (projectile.impactSound) this.add(lines, `WRITE_ASCII 0x18 ~${projectile.impactSound}~ #8`, 1);
            if (projectile.sourceAnimation) this.add(lines, `WRITE_ASCII 0x20 ~${projectile.sourceAnimation}~ #8`, 1);
            if (projectile.particleColor) this.add(lines, `WRITE_SHORT 0x28 ${projectile.particleColor}`, 1);
            if (projectile.projectileWidth) this.add(lines, `WRITE_SHORT 0x2a ${projectile.projectileWidth}`, 1);
            ...
            if (projectile.stringRef) this.add(lines, `WRITE_LONG 0x30 RESOLVE_STR_REF(@${projectile.stringRef})`, 1);
            if (projectile.color) {
                this.add(lines, `WRITE_SHORT 0x3a ${projectile.color}`, 1);
            }
            if (projectile.colorSpeed) this.add(lines, `WRITE_SHORT 0x38 ${projectile.colorSpeed}`, 1);
            if (projectile.screenShakeAmount) this.add(lines, `WRITE_SHORT 0x3a ${projectile.screenShakeAmount}`, 1);
            if (projectile.idsTarget1) this.add(lines, `WRITE_SHORT 0x3e ${projectile.idsTarget1}`, 1);
            if (projectile.idsTarget2) this.add(lines, `WRITE_SHORT 0x40 ${projectile.idsTarget2}`, 1);
            if (projectile.defaultSpell) this.add(lines, `WRITE_ASCII 0x44 ~${projectile.defaultSpell}~ #8`, 1);
            if (projectile.successSpell) this.add(lines, `WRITE_ASCII 0x4c ~${projectile.successSpell}~ #8`, 1);
            ...
            if (projectile.projectileSmokeAnimation) this.add(lines, `WRITE_SHORT 0x134 ${projectile.projectileSmokeAnimation}`, 1);
            ...
            if (projectile.rayCount) this.add(lines, `WRITE_SHORT 0x202 ${projectile.rayCount}`, 1);
            if (projectile.triggerRadius) this.add(lines, `WRITE_SHORT 0x204 ${projectile.triggerRadius}`, 1);
            if (projectile.areaOfEffect) this.add(lines, `WRITE_SHORT 0x206 ${projectile.areaOfEffect}`, 1);
            if (projectile.fragmentAnimation) this.add(lines, `WRITE_SHORT 0x212 ${projectile.fragmentAnimation}`, 1);
            if (projectile.explosionEffect) this.add(lines, `WRITE_BYTE 0x217 ${projectile.explosionEffect}`, 1);
            if (projectile.triggerCount) this.add(lines, `WRITE_BYTE 0x216 ${projectile.triggerCount}`, 1);
            if (projectile.coneWidth) this.add(lines, `WRITE_SHORT 0x224 ${projectile.coneWidth}`, 1);
            this.add(lines, `ADD_PROJECTILE ~override/${projectile.file}.pro~ ~${projectile.description}~`, 0);
            this.add(lines, '', 0);
        }
    }

    private createGrabSpell(lines: CodeLine[], creature: Creature, grab: GrabFullConfig) {
        ...
        this.add(lines, `SAY NAME1 @${grab.grabDisplayStringRef} SAY NAME2 @${grab.grabDisplayStringRef}`, 1);
    ...
    private addGrabEffect(lines: CodeLine[], tab: number, grab: GrabFullConfig, type: 'SPL' | 'ITM') {
        const effect = this.grabService.getCastSpellGrabEffect(grab);
        this.addEffect(lines, tab, effect, type);
    }

    private addEffect(lines: CodeLine[], tab: number, effect: Effect, type: 'SPL' | 'ITM' | 'CRE') {
        if (effect.opcode === EffectTypeEnum.RemoveSpellTypeProtections) {
            this.add(lines, `LPF GET_2DA_ENTRY_OF INT_VAR col_match = 0 STR_VAR file = ~msectype.2da~ entry_match = ~${effect.parameter2}~ RET row col END`, 1);
        ...
        if (effect.parameter1) this.add(lines, `parameter1 = ${this.getIntegerValue(effect.parameter1)}`, tab + 2);
        if (effect.parameter2) this.add(lines, `parameter2 = ${this.getIntegerValue(effect.parameter2)}`, tab + 2);
        if (effect.timing) this.add(lines, `timing = ${effect.timing}`, tab + 2);
        if (effect.dispelResistance) this.add(lines, `resist_dispel = ${effect.dispelResistance}`, tab + 2);
        if (effect.duration) this.add(lines, `duration = ${effect.duration}`, tab + 2);
        this.add(lines, `probability1 = ${effect.probability1}`, tab + 2);
        if (effect.probability2) this.add(lines, `probability2 = ${effect.probability2}`, tab + 2);
        if (effect.diceThrown) this.add(lines, `dicenumber = ${effect.diceThrown}`, tab + 2);
        if (effect.diceSize) this.add(lines, `dicesize = ${effect.diceSize}`, tab + 2);
        if (effect.saveTypes) {
        ...
        if (effect.flags !== undefined) {
            const special = typeof effect.flags === 'number' ? effect.flags : effect.flags.reduce((sum, save) => {
                sum += 2 ** save;
                return sum;
            }, 0);
    ...
    private patchCreatures(lines: CodeLine[], creature: Creature) {
        ...
        if (creature.grab) this.add(lines, `ADD_MEMORIZED_SPELL ~${creature.grab.file}~ #0 ~innate~ (1)`);
        for (const name of creature.additionalData.immunities) {
            const immunity = State.immunities.find(i => i.name === name) as ImmunityConfig;
            if (!immunity.itemSlot) {
                this.add(lines, `LPF ${this.utils.getImmunityFunctionName(name)} END`, 3);
            }
        }
        this.patchCreature({
            lines,
    ...
    private removeKnownSpells(lines: CodeLine[], tab: number, creature: Creature) {
        const files = [...creature.adjustments.reduce((acc, adjustement) => {
    ...
    private removeMemorizedSpells(lines: CodeLine[], tab: number, creature: Creature) {
        ...
        this.executeCodeWithExcludedFiles(lines, tab, `REMOVE_MEMORIZED_SPELLS`, files);
    }

    private removeItems(lines: CodeLine[], tab: number, additionalData: CreatureAdditionalData) {
    ...
    private addProficiencies(lines: CodeLine[], tab: number, additionalData: CreatureAdditionalData) {
        if (!additionalData.proficiencies.length) return;
        for (const prof of additionalData.proficiencies) this.add(lines, `SET_BG2_PROFICIENCY ~${prof.type}~ ${prof.value}`, tab);
    }

    private addItemSlots(lines: CodeLine[], tab: number, additionalData: CreatureAdditionalData) {
        for (const item of additionalData.itemSlots) {
            const equip = item.slot === ItemSlotEnum.WEAPON1 ? 'EQUIP' : '';
            this.add(lines, `ADD_CRE_ITEM ~${item.file}~ #0 #0 #0 ~UNDROPPABLE~ ~${item.slot}~ ${equip}`, tab);
        }
    }

    private addMemorizedSpells(lines: CodeLine[], tab: number, additionalData: CreatureAdditionalData, spells: Spell[]) {
        for (const m of additionalData.memorizedSpells) {
            ...
            this.add(lines, `ADD_MEMORIZED_SPELL ~${m.file}~ #${spell ? (spell.spellLevel ?? 1) - 1 : infos.level - 1} ~${infos.type}~ (${m.memorizedCount})`, tab);
        }
    }
    ...
    private patchCreature(p: {
        ...
        this.creatureService.autogenerateData({ data: p.data, autoGenerate: p.autoGenerate, parent: p.parent, creature: p.creature });
        ...
        for (const key of CREATURE_DATA_KEYS) {
            if (p.data[key] !== undefined) {
                ...
                if (value !== undefined) this.add(p.lines, `${key}=${this.getIntegerValue(value)}`, p.tab + 2);
                if (key === 'gender') {
        ...
        if (p.creature.dualWielding && !p.parent) this.add(p.lines, `perfect2weapon=1`, p.tab + 2);
        this.add(p.lines, 'END', p.tab);
    }

    private patchScripts(lines: CodeLine[], tab: number, creature: Creature) {
        const summonFiles = [...new Set(creature.adjustments.filter(a => a.summon).map(a => a.files).flat())];
        const locationFiles = [...new Set(creature.adjustments.filter(a => !!a.additionalData.scriptLocation).map(a => a.files).flat())];
        const noScriptFiles = [...new Set(creature.adjustments.filter(a => a.noScript).map(a => a.files).flat())];
        const scriptName = `${this.extractScriptName(creature.bafFile as string)}`;
        const summonScriptName = `${this.extractScriptName(creature.bafFile as string)}su`;
        this.patchScript({
            lines,
            tab,
            script: scriptName,
    ...
    private patchScript(p: {
        ...
        if (p.removeScripts.length) {
            this.add(p.lines, `DEFINE_ARRAY removeScripts BEGIN ${p.removeScripts.join(' ')} END`, p.tab);
            removeScripts = ' removeScripts';
        }
        if (p.skipFiles.length) {
            this.add(p.lines, `DEFINE_ARRAY skipFiles BEGIN ${p.skipFiles.join(' ')} END`, p.tab);
            skipFiles = ' skipFiles';
        }
        if (p.files.length) {
            this.add(p.lines, `DEFINE_ARRAY files BEGIN ${p.files.join(' ')} END`, p.tab);
            files = ' files';
        }
        const slot = p.slot ? ` slot=${p.slot}` : '';
        this.add(p.lines, `LPF patchCreatureScript STR_VAR script=${p.script}${slot}${files}${skipFiles}${removeScripts} END`, p.tab);
    }

    private handleAdjustments(lines: CodeLine[], tab: number, creature: Creature) {
        for (const adjustment of creature.adjustments) {
            for (const f of adjustment.files) if (!creature.files.includes(f)) throw new Error(`Unknown adjustment file ${f}`);
            if (adjustment.data
    ...
    private handleAdjustment(lines: CodeLine[], tab: number, creature: Creature, adjustment: CreatureAdjustment) {
        for (const f of adjustment.files) if (!creature.files.includes(f)) throw new Error(`Unknown adjustment file ${f}`);
        const fileEquals = adjustment.files.map(f => `("%SOURCE_RES%" STRING_EQUAL_CASE ~${f}~)`);
        ...
        if (adjustment.data) this.patchCreature({
            lines,
            tab,
        ...
        if (adjustment.additionalData) {
            ...
            this.addMemorizedSpells(lines, tab, adjustment.additionalData, creature.spells);
    ...
    private extractDataValue(key: keyof CreatureData, creature: CreatureData) {
        if (['alignment', 'animation', 'general', 'race', 'class', 'gender'].includes(key))
            return this.utils.getIdsValue(key, creature[key] as string);
