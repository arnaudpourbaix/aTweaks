...
export class WeiduFunctionService extends AbstractWeiduService {
    ...
    generateFunctions(): void {
        ...
        fs.writeFileSync(State.config.commonFunctionsFile, content);
    }

    generateImmunityFunction(lines: CodeLine[], immunity: ImmunityConfig, tab: number): void {
        this.add(lines, `DEFINE_PATCH_FUNCTION ${this.utils.getImmunityFunctionName(immunity)}`, tab);
        ...
        if (immunity.preventEffects.length
        ...
        for (const type of immunity.immunities) {
            this.add(lines, `LPF ${this.utils.getImmunityFunctionName(type)} END`, tab + 1);
        }
        this.add(lines, `END`, tab);
        this.add(lines, ``);
        if (immunity.itemSlot) this.weiduCoreService.generateItem(immunity.itemSlot, immunity);
    }
    ...
    callImmunityFunction(lines: CodeLine[], immunity: ImmunityConfig, tab: number): void {
        ...
        const display = immunity.displaySpellIneffective ? ' INT_VAR displaySpellIneffective=1' : '';
        this.add(lines, `LPF ADD_IMMUNITY_CRE_ITM_SPL${display} STR_VAR${effects}${icons}${strings}${animations}${spells} END`, tab);
    }

    generateEffect(lines: CodeLine[], effect: Effect, tab: number): void {
        const parameter1 = effect.parameter1 !== '0' ? ` parameter1=${this.getIntegerValue(effect.parameter1)}` : '';
        const parameter2 = effect.parameter2 !== '0' ? ` parameter2=${this.getIntegerValue(effect.parameter2)}` : '';
        const special = !!effect.special ? ` special=${this.getIntegerValue(effect.special)}` : '';
    ...
    generateSpells(lines: CodeLine[], immunity: ImmunityConfig, tab: number): string {
        ...
        if (immunity.idsSpells) {
            this.add(lines, `DEFINE_ARRAY identifiers BEGIN ${immunity.idsSpells.map(i => i.id).join(" ")} END`, tab);
            this.add(lines, `PATCH_PHP_EACH identifiers AS _ => ids BEGIN`, tab);
            this.add(lines, `LPF getResourceFromSpellIds STR_VAR ids RET result END`, tab + 1);
