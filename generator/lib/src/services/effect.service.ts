    ...
    const results: Effect[] = effects.reduce((acc, effect) => {
      ...
      switch (result.opcode) {
        ...
        case EffectTypeEnum.Thac0VsCreatureTypeModifier:
          result.parameter1 = `IDS_OF_SYMBOL (~${
            (<IdsEffect>effect).idsFile
          }~ ~${(<IdsEffect>effect).idsEntry}~)`;
          result.parameter2 = `${
            EffectIDSFileEnum[(<IdsEffect>effect).idsFile]
          }`;
          break;
        ...
        case EffectTypeEnum.Disease:
          ...
          result.parameter2 = `${
            DiseaseTypeEnum[(<DiseaseEffect>effect).type]
          }`;
        ...
        case EffectTypeEnum.KillTarget:
          result.parameter1 = `${
            (<KillTargetEffect>effect).displayText ? 0 : 1
          }`;
          result.parameter2 = `${
        ...
        case EffectTypeEnum.DispelEffects:
          if ((<DispelEffectsEffect>effect).dispelType)
            result.parameter1 = `${
              DispelEffectTypeEnum[
                (<DispelEffectsEffect>effect).dispelType ?? 0
              ]
            }`;
      ...
      acc.push(result);
      if (
        result.parameter1 === "undefined" ||
        result.parameter2 === "undefined"
      )
        throw new Error(`Invalid effect: ${JSON.stringify(result)}`);
      return acc;
    }, [] as Effect[]);
    return results;
  }
