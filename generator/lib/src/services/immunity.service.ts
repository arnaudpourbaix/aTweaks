...
export class ImmunityService {
    ...
    private checkImmunities(additionalData: CreatureAdditionalData, creature: Creature): void {
        for (const name of additionalData.immunities) {
            ...
            if (immunity.itemSlot) {
                ...
                if (overwrittingItem) console.log(chalk.yellowBright(`${figureSet.arrowRight} skipping ${immunity.itemSlot.file} because ${overwrittingItem.file} overwrites it`));
                else additionalData.itemSlots.push({ file: immunity.itemSlot.file, slot: immunity.itemSlot.slot });
            }
        }
    }
